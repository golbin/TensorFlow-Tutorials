...
class Brain:
    ...
    def __init__(self, session, width, height, n_action):
        ...
        # 게임판의 크기는 84x84 로 하고 gray scale 을 이용합니다.
        # 학습하기에 사이즈가 너무 크기 때문입니다.
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            model = tf.layers.conv2d(self.input_X, 32, [8, 8], activation=tf.nn.relu)
            model = tf.layers.max_pooling2d(model, [2, 2], [2, 2], padding='SAME')
            model = tf.layers.conv2d(model, 64, [4, 4], activation=tf.nn.relu)
            model = tf.layers.conv2d(model, 64, [3, 3], activation=tf.nn.relu)
